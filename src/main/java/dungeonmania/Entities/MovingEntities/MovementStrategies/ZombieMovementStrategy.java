...
public class ZombieMovementStrategy extends MovementStrategy {
    @Override
    public void move() {
        if (Dungeon.getPlayer().activePotionEffect().equals("invincibility_potion")) {
            runAway();
            return;
        }
    ...
    private void runAway() {
        MovingEntity zombie = (MovingEntity) getEntity();
        zombie.setMovementStrategy(new PlayerInvincibleMovevmentStrategy());
        zombie.getMovementStrategy().setEntity(zombie);
        zombie.getMovementStrategy().move();
        zombie.setMovementStrategy(this);
    }
